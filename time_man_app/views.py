...
def homepage(request):
    ...
    collection_id = Collection.objects.get(title = "General", user_id = request.session['user_id']).id
    collections = Collection.objects.all().filter(user_id = request.session['user_id']).exclude(title = "General")
    context = {
        "user": User.objects.get(id=user_id),
        "general_tasks": Task.objects.filter(user_id = user_id, collection_id = collection_id).order_by('-created_at'),
...
def createCollection(request):
    if "user_id" not in request.session:
        ...
        return redirect('/')
    post = request.POST
    errors = Collection.objects.basic_validator(post)
    if len(errors) > 0:
        print(errors.items())
        for key, value in errors.items():
            messages.error(request, value)
